...
import RestAssured.model.data.UserPOJO_Lombok_Builder;
...
public class DemoBaseTest extends BaseTest {
    ...
    public void testUpdateUser_PATCH(){
        ...
        Response response = request.when().patch("/user/339");
...
@Test
    public void testUpdateUser_PATCH_Builder(){
        loginUser();
        RegisterUserPOJO_Lombok registerUserPOJOLombok = UserPOJO_Lombok_Builder.getUserData();
        Gson gson = new Gson();
        RequestSpecification request = given();
        request.baseUri(ConfigsGlobal.URI)
                .accept("application/json")
